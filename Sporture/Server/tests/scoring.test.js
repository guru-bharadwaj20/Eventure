...
describe("urgencyScore", () => {
  ...
  it("never leaves the [0, 1] range", () => {
    for (const [taken, max, days] of [[0, 10, 0], [5, 10, 1], [9, 10, 60], [0, 2, 365]]) {
...
const user = { favSports: ["Football"], skillLevel: "Intermediate" };
...
describe("scoreEvent", () => {
  ...
  it("handles a user with no preferences at all", () => {
    const blank = { favSports: [], skillLevel: undefined };
...
describe("rankEvents", () => {
  it("orders by descending score", () => {
    ...
    expect(ranked.map((e) => e._id)).toEqual(["near", "mid", "far"]);
  ...
  it("orders identically regardless of input order", () => {
    ...
    const forward = rankEvents(events, user, { now }).map((e) => e._id);
    const reversed = rankEvents([...events].reverse(), user, { now }).map((e) => e._id);
  ...
  it("preserves the original event fields", () => {
    const [ranked] = rankEvents([makeEvent()], user, { now });
