 ...
 */
export const sportAffinity = (favSports, sport) => {
 ...
 */
export const proximityScore = (distanceMetres, halfLifeMetres = 5000) => {
 ...
 */
export const socialSignal = (pastTeammateIds, playerIds) => {
 ...
 */
export const urgencyScore = (spotsTaken, maxPlayers, eventDate, now = new Date()) => {
  const days = (new Date(eventDate) - now) / 86400000;
 ...
 *
 * @param {object} event  Event with location, sport, date, players, host.
 * @param {object} user   Viewer: favSports, skillLevel.
 * @param {object} ctx    { pastTeammateIds: Set<string>, now: Date }
 * @returns {{ score: number, breakdown: object, reasons: string[] }}
 */
export const scoreEvent = (event, user, ctx = {}) => {
  const { pastTeammateIds = new Set(), now = new Date() } = ctx;

  const playerIds = (event.currentPlayers || []).map((p) => p?._id ?? p);
  ...
  const score =
    Object.entries(signals).reduce((sum, [k, v]) => sum + v * WEIGHTS[k], 0) / totalWeight;
 ...
 */
const buildReasons = (signals, event, user, pastTeammateIds, playerIds, now) => {
  const reasons = [];
  ...
  const days = Math.ceil((new Date(event.date) - now) / 86400000);
 ...
 */
export const rankEvents = (events, user, ctx = {}) =>
  events
